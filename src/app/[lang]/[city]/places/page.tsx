...
export async function generateMetadata({
  ...
  return {
    // title каталога с ключевым запросом (совпадает с h1); бренд-суффикс
    // добавляет template из [lang]/layout
    title: dict.places.heroTitle,
    // self-canonical каталога: ?page=/?view=map/?age= не плодят дубли
    alternates: pageAlternates(lang, citySlug, "/places"),
  };
}
...
export default async function CityPlacesPage({
  ...
  // ссылки самого списка (фильтры/пагинация/сброс) — на /places; ссылки на
  // карточки и разделы по-прежнему строятся от корня города (basePath)
    ...
    <main className="page-shell">
      ...
      {total === 0 ? (
        <section className="empty-state">
          ...
          <Link href={listPath} className="empty-state-cta">
