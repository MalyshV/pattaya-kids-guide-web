...
/**
 * OCR-зона админки: скрин/фото → распознанный текст через onText.
 * У событий текст уходит в поле «Разобрать афишу» (парсер раскидает по
 * полям), у мест и занятий — в черновик для копирования (OcrScratchpad);
 * следующий шаг у каждой формы свой — он приходит через doneMessage.
 *
 ...
 * текст попадает в редактируемое поле, человек правит сам.
 *
...
export function OcrZone({
  subject,
  doneMessage,
  onText,
}: {
  /// начало подписи зоны: «Скрин афиши», «Скрин расписания или прайса»…
  subject: string;
  /// следующий шаг после распознавания — законченное предложение
  doneMessage: string;
  ...
  return (
    <div className="admin-ocr">
      ...
      >
        {working
          ? phase.message
          : `${subject}: нажмите, чтобы выбрать файл, перетащите сюда или вставьте ⌘V`}
      </button>
      <input
        ref={inputRef}
        type="file"
