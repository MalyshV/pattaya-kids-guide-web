  ...
  suggest: {
    ...
    photos: {
      ...
      unreadable: (names: readonly string[]): string =>
        `เปิดไฟล์นี้เป็นรูปภาพไม่ได้: ${names.map((name) => `“${name}”`).join(", ")} ลองใช้ไฟล์อื่น เช่น JPG หรือ PNG`,
      rightsLabel: "เป็นรูปของฉันเอง หรือฉันมีสิทธิ์แชร์รูปเหล่านี้",
      rightsRequired: "กรุณาติ๊กช่องนี้ หรือนำรูปออก",
    ...
    formErrors: {
      ...
      photos:
        "รับรูปไม่สำเร็จ ลองอีกครั้ง หรือนำรูปออกแล้วส่งโดยไม่มีรูป ข้อมูลอื่นยังอยู่ครบ",
    },
    similar: {
