...
export const ru = {
  ...
  errorPage: {
    ...
    description:
      "Мы уже знаем о сбое. Попробуйте обновить страницу — обычно это помогает.",
  ...
  common: {
    ...
    // невидимая приписка для скринридера у ссылок, открывающих новую вкладку
    ...
    // карусель галереи: стрелки листания и счётчик «2 из 4»
    prevPhoto: "Предыдущее фото",
    nextPhoto: "Следующее фото",
    photoCounter: (current: number, total: number) => `${current} из ${total}`,
  ...
  memory: {
    // в шапке — две отдельные точки входа: обе функции видны без догадок
    navSaved: "Сохранённое",
    navVisited: "Были здесь",
    pageTitle: "Избранное",
    pageIntro:
      "Места, занятия и события, которые вы сохранили или где уже были. Список хранится только в этом браузере — без регистрации.",
    backToCatalog: "← К местам",
    emptyCta: "Смотреть места",
    saveLabel: "Сохранить",
    savedLabel: "Сохранено",
    saveAria: "Сохранить в избранное",
    savedAria: "Убрать из сохранённого",
    visitLabel: "Были здесь",
    // подпись меняется после нажатия: глиф ✓ один — без смены текста родитель
    // не видел, что сработало, тапал второй раз и снимал отметку
    visitedLabel: "Отмечено: были",
    visitAria: "Отметить «были здесь»",
    visitedAria: "Убрать отметку «были здесь»",
    savedSection: "Сохранённое",
    visitedSection: "Были здесь",
    savedSectionEmpty: "Пока пусто — жмите ♡ на карточках, чтобы сохранить на будущее.",
    visitedSectionEmpty: "Пока пусто — отмечайте ✓ на карточках мест, где уже были.",
    emptyTitle: "Пока пусто",
    emptyHint:
      "Нажимайте ♡ на карточках, чтобы сохранить, и ✓ — чтобы отметить, где уже были. Всё появится здесь.",
    remove: "Убрать",
    removed: "Убрано из списка",
    restore: "Вернуть",
    entityPlace: "Место",
    entityActivity: "Занятие",
    entityEvent: "Событие",
    // фильтр каталога по отметкам ✓ — появляется, только когда отметки есть
    filterTitle: "Отметки «были здесь»:",
    filterAll: "Показывать все",
    filterHide: "Скрыть посещённые",
    filterOnly: "Только посещённые",
    filterHiddenNote: (count: number) => `Посещённые скрыты: ${count}`,
    filterOnlyNote: (count: number) =>
      count === 1
        ? "Показано только посещённое: 1 место"
        : `Показаны только посещённые: ${count}`,
    filterEmptyOnlyTitle: "Среди найденного нет посещённых мест",
    filterEmptyOnlyHint:
      "Отметки ✓ «были здесь» ставятся на карточках мест — пока ни одно из этих мест не отмечено.",
    filterEmptyHiddenTitle: "Все найденные места уже посещены",
    filterEmptyHiddenHint:
      "Каждое место из этих результатов отмечено ✓ «были здесь» — снимите фильтр, чтобы их увидеть.",
    filterEmptyCta: "Показывать все",
  },
  ...
  places: {
    ...
    // возраст живёт не в блоке «Фильтры» — общий совет указывал бы не туда
    emptyAgeHint:
      "Для этого возраста пока ничего не нашлось — попробуйте другой возраст или снимите выбор.",
    emptyCta: "Показать все места",
    ...
    viewList: "Список",
    viewMap: "Карта",
    // имя региона карты для скринридера (сама карта — доп. вид, основной
    // доступный путь — список с теми же фильтрами)
    mapRegionLabel: "Карта мест",
  ...
  search: {
    placeholder: "Найти место, занятие или событие…",
    ariaLabel: "Поиск по местам, занятиям и событиям",
  ...
  birthdays: {
    ...
    heroDescription:
      ...
      "Площадки, где проводят детские дни рождения: пакеты, цены, депозиты и контакты — собираем и проверяем на месте.",
    askOnBooking: "— спросите при бронировании",
    ageNote: "Возраст здесь не фильтрует — показываем все площадки для дня рождения.",
    guestsLabel: "Гости:",
  ...
  eventFilters: {
    ...
    labels: {
      ...
      // «Сейчас» читалось как «сегодня» — а вкладка показывает только уже
      // ИДУЩИЕ многодневные события; подпись должна обещать ровно это
      ongoing: "Идут сейчас",
      past: "Прошедшие",
    },
  },
